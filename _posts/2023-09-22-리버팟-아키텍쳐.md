...
- 이때 한가지 규칙을 정한다! ~~View 에서만 controller 를 watch 하고, Component 는 View 에 종속되어 View 로부터 data 를 전달받아야 한다.~~
  - (수정_202310041947) Component 에서도 Controller 를 적극적으로 watch 하자! 상위 위젯에서 하위 위젯으로 상태를 일일이 전달해야하는 비효율성을 개선하기 위해 나온 것이 Riverpod 이다(내생각). 따라서 하위 위젯인 Component 에서도 Controller 또는 Provider 를 자유롭게 watch 하고, 메서드를 호출하는 것이 바람직하다!! (전문가도 이렇게 했음)
